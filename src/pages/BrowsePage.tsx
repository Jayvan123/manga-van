...
export default function BrowsePage() {
  ...
  const options = useMemo(() => ({ query: debouncedQuery, includedTagIds: selectedTags, sort, page, limit: 24 }), [debouncedQuery, page, selectedTags, sort])
  ...
  return (
    <div className="container page browse-page">
      <div className="page-heading"><p className="eyebrow">Find your next read</p><h1>Browse manga</h1><p>Search, filter, and sort English manga available on MangaDex.</p></div>
      <BrowseFilters onChange={update} query={query} selectedTags={selectedTags} sort={sort} tags={tags} />
      ...
      {manga.isLoading && <div className="manga-grid">{Array.from({ length: 12 }, (_, index) => <div className="manga-card skeleton-card" key={index} />)}</div>}
      ...
      {manga.data && manga.data.items.length > 0 && <><div className="manga-grid">{manga.data.items.map((item: Manga) => <MangaCard key={item.id} manga={item} progress={progressByManga[item.id]} />)}</div><Pagination limit={24} onChange={(nextPage: number) => { update({ page: nextPage }); window.scrollTo({ top: 0, behavior: 'smooth' }) }} page={page} total={manga.data.total} /></>}
    </div>
  )
}

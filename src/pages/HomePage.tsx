...
function CategorySection({ category, tags, progressByManga }: CategorySectionProps) {
  ...
  return <MangaSection action={<Link to={`/browse?tag=${tag?.id || ''}`}>View all</Link>} icon={category.icon} progressByManga={progressByManga} query={{ ...query, isLoading: !tag || query.isLoading }} title={category.name} />
}
    ...
    <>
      <section className="hero-banner">
        <div className="container hero-banner__content">
          ...
          <p>Discover thousands of manga, track your place without an account, and read anywhere.</p>
          <div className="hero-banner__actions">
            <Link className="button button--primary" to="/browse">Explore manga</Link>
      ...
      <div className="container home-content">
        {recentManga.length > 0 && (
          <section className="content-section">
            <div className="section-heading"><h2><span aria-hidden="true">◷</span> Recently read</h2></div>
            <div className="manga-row">{recentManga.map((manga) => <MangaCard key={manga.id} manga={manga} progress={progressByManga[manga.id]} />)}</div>
          </section>
        )}

        <MangaSection progressByManga={progressByManga} query={{ ...recommendations, data: dailyRecommendations }} title="For You" icon="✦" action={<Link to="/browse?sort=popularity">More picks</Link>} />
        <div id="latest"><MangaSection progressByManga={progressByManga} query={latest} title="Latest Updates" icon="⚡" action={<Link to="/browse?sort=latest">View all</Link>} /></div>
        {CATEGORIES.map((category) => <CategorySection category={category} key={category.name} progressByManga={progressByManga} tags={tags} />)}

        <section className="content-section">
          <div className="section-heading"><div><h2>All Manga</h2><p>Browse the MangaDex catalog</p></div><Link to="/browse">Advanced filters</Link></div>

...
// Lightweight, high-performance take on Magic UI's SparklesText.
// Instead of using React intervals which cause heavy DOM manipulation and lag on mobile,
// this generates a stable set of random coordinates once, and runs infinitely on CSS
// using hardware acceleration (transform/opacity with will-change).
...
export default function SparklesText({ children, className = '', sparklesCount = 8, colors = {} }) {
  ...
  // Generate a stable list of sparkles once (or when colors/count change).
  // Zero React state updates after mount means zero layout thrashing or CPU overhead.
  const sparkles = useMemo(() => {
    return Array.from({ length: sparklesCount }, () => generateSparkle(first, second))
  }, [sparklesCount, first, second])
